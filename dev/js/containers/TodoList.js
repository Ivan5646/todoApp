        ...
        return this.props.todoList.map((item) => { 
            return (
                <li key={item.id} className="task"> 
                    ...
                    {this.state.showForm && this.state.showId==item.id && <EditItem id={item.id} name={item.name} description={item.description} priority={item.priority} due_date={item.due_date} complete_date={item.complete_date} />}
                </li>
            );
        });
    }
